label Showering(occupants=[], StayCount=[] , Showered=0, Line=0, temp_Girls=[]):
    ...
    while temp_Girls:
        ...
        if temp_Girls[0].location == "bg_showerroom" and temp_Girls[0] not in occupants:
            $ occupants.append(temp_Girls[0])
        ...
        if len(occupants) > len(StayCount):
            ...
            menu:
                ...
                "But I didn't get to watch." if Showered:
                    $ Line = "watch you"
            if Line:
                $ temp_Girls = occupants[:]
                while temp_Girls:

                    if temp_Girls[0].location == bg_current and temp_Girls[0] not in StayCount:
                        if temp_Girls[0] == EmmaX and (not "classcaught" in EmmaX.history or (StayCount and "three" not in EmmaX.history)):

                            pass
                        elif temp_Girls[0] == JeanX and approval_check(temp_Girls[0], 600):
                            $ StayCount.append(temp_Girls[0])
                        elif temp_Girls[0] == StormX:
                            if approval_check(temp_Girls[0], 700, "LO"):
                                $ StayCount.append(temp_Girls[0])
                        elif approval_check(temp_Girls[0], 1200,Alt=[[KittyX],1400]) or (approval_check(temp_Girls[0], 600,Alt=[[KittyX],700]) and temp_Girls[0].SeenChest and temp_Girls[0].SeenPussy):
                            $ StayCount.append(temp_Girls[0])
    ...
    if Nearby and len(StayCount) < 2:
        ...
        if EmmaX in StayCount and "three" not in EmmaX.history:
            ...
            if len(temp_Girls) >= 2:
                ...
                "Seeing [temp_Girls[0].name] arrive, [EmmaX.name] quickly excuses herself."
        ...
        if temp_Girls:
            ...
            if approval_check(temp_Girls[0], 1200):
                $ StayCount.append(temp_Girls[0])
            if len(temp_Girls) >=2 and approval_check(temp_Girls[1], 1200) and len(StayCount) < 2:
                $ StayCount.append(temp_Girls[1])

            if len(temp_Girls) >=2:
                if temp_Girls[0] not in StayCount and temp_Girls[1] not in StayCount:
                    "They both turn right back around."
                    call remove_girl (temp_Girls[0])
                    call remove_girl (temp_Girls[1])
                    $ temp_Girls = []
                elif temp_Girls[0] not in StayCount:
                    "[temp_Girls[0].name] turns right back around, but [temp_Girls[1].name] stays."
                    call remove_girl (temp_Girls[0])
                    $ temp_Girls.remove(temp_Girls[0])
                elif temp_Girls[1] not in StayCount:
                    "[temp_Girls[1].name] turns right back around, but [temp_Girls[0].name] stays."
                    call remove_girl (temp_Girls[1])
                    $ temp_Girls.remove(temp_Girls[1])
            elif temp_Girls[0] not in StayCount:
