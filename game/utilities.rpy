...
label Wait (Outfit = 1, Lights = 1, Girls=[]):
    ...
    while Girls:
            ...
            if Girls[0].Loose < 2:  #checks how tight the girl's asshole is
                        if (Girls[0].action_counter["anal"] + Girls[0].action_counter["dildo_ass"] + Girls[0].Plug) >= 15:
                                $ Girls[0].Loose = 2
                        elif (Girls[0].action_counter["anal"] + Girls[0].action_counter["dildo_ass"] + Girls[0].Plug) >= 3:
                                $ Girls[0].Loose = 1
...
label Girls_Schedule(Girls = [], Clothes = 1, location = 1, locationTemp = 0):
        ...
        while Girls:
                if Girls[0] in Party and Clothes != 2 or not location:
                        ...
                        if Girls[0].location != locationTemp and Girls[0] not in Party:
                                #if she moved
                                if locationTemp == bg_current:
...
label clear_the_room(Girl = None, passive = False, silent = False, Girls_to_remove = []):
    ...
    while other_Girls:
        ...
        elif other_Girls[0].location == "bg_teacher" and bg_current == "bg_classroom":
            $ Girls_to_remove.append(other_Girls[0])
...
label Girls_location(GirlsNum = 0, Change=0, other_Girls=[]):
        ...
        $ other_Girls = all_Girls[:]
        $ renpy.random.shuffle(other_Girls)
        while other_Girls:
                #cycles through each girl possible, adds them to the local area if possible
                if "leaving" in other_Girls[0].recent_history:
                        if "sleepover" in other_Girls[0].Traits:
                                $ other_Girls[0].DrainWord("sleepover",0,0,1)  #remove from Traits
                        call leave(other_Girls[0])
                        if other_Girls[0].location != bg_current:
                                if other_Girls[0] in Present:
                                        $ Present.remove(other_Girls[0])
                                $ Change = 1
                        $ GirlsNum += 1
                #if Girl was in Nearby, but was moved to a new location
                if other_Girls[0] in Nearby and other_Girls[0].location != "nearby": #and other_Girls[0].location != bg_current
                                $ Nearby.remove(other_Girls[0])
                $ other_Girls.remove(other_Girls[0])
        ...
        $ other_Girls = all_Girls[:]
        while other_Girls:
                        if "arriving" in other_Girls[0].recent_history:
                                call Girls_Arrive
                                return
                        $ other_Girls.remove(other_Girls[0])
        return
...
label Taboo_Level(Taboo_location=1,Teach=0,Girls=[]): #rkeljsv
        #cycles through each girl, setting their taboo level.
        # if Taboo_location, will only work on local Girls.
        ...
        while Girls:
                ...
                if not Taboo_location or Taboo_check == bg_current:
...
label Partner_Like(Girl=0,Value=1,AltValue=1,Measure=800,Partner=Partner):
        ...
        if second_girl_primary_action:
                ...
                if second_girl_primary_action == "watch":
                        ...
                        $ Value += 2
        #End second_girl_primary_action bonuses

        $ Partner.GLG(Girl,Measure,Value,1)
        $ Girl.GLG(Partner,Measure,Value,1)
        return

label GirlWaitUp(locational=0,check=70,D20=0,Teach=0,GirlsA=[],GirlsB=[]):  #rkeljsv
        #This adjusts girl's liking each other based on shared activities
        #locational =1 only checks if they are in the room with you.
...
label Jumped(Act=0): #rkeljsv
        ...
        if Act == "leave":
                ...
                menu:
                    "Go along with it":
                        $ Girls[0].change_stat("Inbt", 95, 3)
                        "You follow after her."
                    "Pull away from her and head back.":
                        $ Girls[0].change_stat("Love", 90, -10)
                        $ Girls[0].change_stat("Obed", 50, 10)
                        $ Girls[0].change_stat("Obed", 95, 5)
                        $ Girls[0].change_stat("Inbt", 95, -5)
            ...
            menu:
                    "See where this is going":
                        $ Girls[0].change_stat("Inbt", 95, 2)
                    "Not here [[head to your room]":
                        $ Girls[0].change_stat("Inbt", 95, 1)
...
label Sex_Dialog(Primary=focused_Girl,Secondary=0,TempFocus=0,Primarylust=0,Secondarylust=0,line1=0,line2=0,line3=0,line4=0,D20S=0):  #rkeljsv
        ...
        if line4:
                ...
                "[line4]"
                if second_girl_primary_action == "suck breasts" or second_girl_primary_action == "fondle_breasts":
...
label primary_action_Swap(Active = 0, primary_actionX1 = primary_action, primary_actionX3 = girl_offhand_action, Primary = Partner): #rkeljsv
    ...
    if second_girl_primary_action:
            ...
            if second_girl_primary_action == "masturbation":
                    ...
                    $ second_girl_primary_action = 0
            elif second_girl_primary_action in ("handjob","blowjob","kiss you"):
...
label Seen_First_Peen(Primary=0, Secondary=0, silent=0, Undress=0, passive=0, GirlsNum=0, React=0, other_Girls=[]):
    ...
    if not Primary:
            ...
            $ other_Girls = Present[:]  #loads up all local girls
            $ renpy.random.shuffle(other_Girls)
            while other_Girls:
                    #cycles through each girl possible,
                    #If girl is around, check to see if she noticed your cock yet
                    if (focused_Girl == other_Girls[0] or D20 >= 10) and "peen" not in other_Girls[0].recent_history:
                            #If other_Girls[0] is the prinary or secondary Girl, and hasn't seen your cock yet, call the thing
                            #call expression other_Girls[0].Tag + "_First_Peen" pass (silent,Undress)
                            call Girl_First_Peen(other_Girls[0],silent,Undress)
                            $ GirlsNum = _return
                    $ other_Girls.remove(other_Girls[0])
...
label Sex_Over(Clothes=1,Girls=0,other_Girls=[]): #rkeljsv
        ...
        while other_Girls:
                if other_Girls[0].location == bg_current:
                        ...
                        $ other_Girls[0].OCount = 0
                        call Girl_Cleanup(other_Girls[0],"after")
                        if Player.Spunk:
                                ...
                                call Girl_CleanCock(other_Girls[0])

                if "nowhammy" not in JeanX.Traits and "saw with Jean" in other_Girls[0].Traits:
                                #if a girl saw you with Jean and got pissed, she forgets. . .
                                $ other_Girls[0].Traits.remove("saw with Jean")
                                $ other_Girls[0].Traits.append("sawJeanW") #got whammied tag
                $ other_Girls.remove(other_Girls[0])
        ...
        if Clothes:
                ...
                while other_Girls:
                        if other_Girls[0].location == bg_current:
                                if other_Girls[0].OutfitChange(Changed=1) == 2:
                                        if Line:
                                            $ Line = Line + " and " + other_Girls[0].name
                                        else:
                                            $ Line = other_Girls[0].name
                        $ other_Girls.remove(other_Girls[0])
                if len(Girls) > 1:
